...
// 子弹类构造函数实现，目标为敌人
Bullet::Bullet(const QPoint& startPos, Enemy* target, int damage, const QString& imagePath, int width, int height, bool shouldSlow)
    : m_startPos(startPos), m_currentPos(startPos), m_enemyTarget(target), m_obstacleTarget(nullptr), m_damage(damage), m_isReachedTarget(false), m_speed(20), m_rotation(0), m_shouldSlow(shouldSlow)
{
...
// 子弹类构造函数实现，目标为障碍物
Bullet::Bullet(const QPoint& startPos, Obstacle* target, int damage, const QString& imagePath, int width, int height)
...
void Bullet::checkAndApplyDamage()
{
    ...
    if ((m_currentPos - targetPos).manhattanLength() <= m_speed) {
        if (m_enemyTarget) {
            ...
            // 减速效果：降低敌人速度50%，持续3秒
            if (m_shouldSlow) {
                m_enemyTarget->applySlow(0.5, 3000);
            }
        } else if (m_obstacleTarget) {
            // 若目标为障碍物，障碍物受到伤害
