...
public:
    ...
    Bullet(const QPoint& startPos, Enemy* target, int damage, const QString& imagePath, int width = 0, int height = 0, bool shouldSlow = false);
    // 构造函数，初始化子弹的起始位置、目标、攻击力和贴图，目标为障碍物
    Bullet(const QPoint& startPos, Obstacle* target, int damage, const QString& imagePath, int width = 0, int height = 0);
    // 析构函数
    ~Bullet();
...
private:
    ...
    bool m_shouldSlow;    // 标记子弹是否带有减速效果
